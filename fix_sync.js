...
let content = fs.readFileSync('src/pages/CampPointsTracker.jsx', 'utf8');
...
// Replace the old syncDB useEffect
content = content.replace(
    /\/\/ DB Sync\s*useEffect\(\(\) => \{[\s\S]*?syncDB\(\);\s*\}, \[\]\);/,
    advancedSync.trim()
);

fs.writeFileSync('src/pages/CampPointsTracker.jsx', content);
