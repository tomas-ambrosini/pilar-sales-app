...
import { MapPin, Navigation, Phone, CheckCircle, Check, Clock, FileText, XCircle, DollarSign, PenTool, Truck, ChevronDown, MessageSquare, MapIcon, AlertTriangle, Wrench } from 'lucide-react';
...
export default function TechnicianMyDay() {
    ...
    const fetchMyDay = async (silent = false) => {
        ...
        try {
            ...
            // Fetch Service Calls (Removed 'metadata' column which does not exist and was crashing the query)
            const { data: svcData, error: svcError } = await supabase.from('service_calls').select(`
                id, status, urgency, call_type, issue_description, scheduled_start, assigned_techs,
            ...
            if (svcError) console.error("Service Calls Query Error:", svcError);
            ...
            if (oppError) console.error("Opportunities Query Error:", oppError);
            ...
            const filteredJobs = combined.filter(job => {
                ...
                if (job.__type === 'SERVICE') {
                    ...
                    belongsToCrew = techsStr.includes(selectedCrewId);
                } else {
                ...
                const timeA = a.scheduled_start ? new Date(a.scheduled_start.replace(' ', 'T').slice(0, 19) + 'Z').getTime() : 0;
                const timeB = b.scheduled_start ? new Date(b.scheduled_start.replace(' ', 'T').slice(0, 19) + 'Z').getTime() : 0;
                return timeA - timeB;
            });
