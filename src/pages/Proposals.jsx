               ...
               return (
                 <div className="w-full">
                    ...
                    {layoutMode === 'kanban' && !loading ? (
                        <div className={`p-6 min-h-[500px] bg-slate-50/30 ${filterMode === 'All' ? 'flex gap-6 overflow-x-auto pb-8 snap-x snap-mandatory hide-scrollbar' : ''}`}>
                 ...
                 <div className="flex flex-col lg:hidden gap-3 p-3 bg-slate-50/30">
                     {filteredProposals.map(proposal => {
                        ...
                        let mainActionText = 'Resume';
                        let mainActionStyle = 'bg-slate-600 text-white hover:bg-slate-700 border border-slate-700 focus:ring-slate-600';
                        if (proposal.status === 'Lead') {
                            mainActionStyle = 'bg-purple-600 text-white hover:bg-purple-700 border border-purple-700 focus:ring-purple-600';
                        } else if (proposal.status === 'Approved') {
                            mainActionText = 'Contract';
                            mainActionStyle = 'bg-emerald-500 text-white hover:bg-emerald-600 border border-emerald-600 focus:ring-emerald-500';
                        } else if (proposal.status === 'Sent') {
                            mainActionText = 'Preview';
                            mainActionStyle = 'bg-blue-600 text-white hover:bg-blue-700 border border-blue-700 focus:ring-blue-600';
                        } else if (proposal.status === 'Lost' || proposal.status === 'Voided') {
                            mainActionText = 'Review';
                            mainActionStyle = 'bg-red-50 text-red-600 hover:bg-red-100 border border-red-200 shadow-none focus:ring-red-500';
                        } else if (proposal.status === 'Pending Void') {
                            mainActionText = ['super_admin', 'admin', 'manager'].includes((user?.role || '').toLowerCase()) ? 'View Request' : 'Undo Request';
                            mainActionStyle = 'bg-amber-100 text-amber-700 hover:bg-amber-200 border border-amber-300 shadow-none focus:ring-amber-500';
                        }

                        return (
                            <div key={proposal.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden active:scale-[0.99] transition-transform cursor-pointer" onClick={() => handleRowClick(proposal)}>
                                <div className="p-4 flex flex-col gap-3">
                                    <div className="flex justify-between items-start">
                                        <div className="flex items-center gap-3 min-w-0 pr-2">
                                            <div className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-slate-600 bg-slate-100 border border-slate-200 shrink-0 shadow-inner">
                                                {proposal.customer?.split(' ').filter(Boolean).map(n => n[0]).join('').substring(0, 2).toUpperCase() || 'C'}
                                            </div>
                                            <div className="flex flex-col min-w-0">
                                                <h3 className="text-sm font-black text-slate-900 truncate leading-tight mb-0.5">{proposal.customer?.replace(/ Account$/i, '').trim()}</h3>
                                                <p className="font-mono text-[10px] font-bold text-slate-400 uppercase tracking-widest truncate">{formatQuoteId(proposal)}</p>
                                            </div>
                                        </div>
                                        <span className={`shrink-0 inline-flex items-center justify-center px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-widest border shadow-sm ${badgeColors}`}>
                                            {proposal.status}
                                        </span>
                                    </div>
                                    <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 flex justify-between items-center mt-1">
                                        <div className="flex flex-col">
                                            {(!proposal.status || ['Draft', 'Lead', 'Sent', 'Opened', 'Pending Void', 'Voided', 'Lost'].includes(proposal.status)) ? (() => {
                                                const { min, max, hasRange } = getEstValueDisplay(proposal);
                                                if (hasRange) {
                                        ...
                                        <div className="flex flex-col text-right">
                                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Last Updated</span>
                                            <span className="text-xs font-semibold text-slate-600">{new Date(proposal.updated_at || proposal.created_at).toLocaleDateString()}</span>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2 mt-1">
                                        <button onClick={(e) => { e.stopPropagation(); handleCopyMessage(proposal); }} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 bg-slate-100 text-slate-600 hover:bg-slate-200 border border-slate-200 rounded-lg text-xs font-bold transition-all shadow-sm">
                                            <Copy size={14} /> Link
                                        </button>
                                        
                                        {proposal.status === 'Approved' && (
                                            <button 
                                                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg shadow-sm transition-all focus:ring-2 outline-none bg-slate-100 text-slate-600 hover:bg-slate-200 border border-slate-300 text-xs font-bold"
                                                onClick={(e) => { e.stopPropagation(); setViewingProposal({ ...proposal, isReadOnly: true }); }}
                                            >
                                                Proposal
                                            </button>
                                        )}
                                        ...
                                        <button 
                                          ...
                                          className={`flex-[1.5] py-2.5 rounded-lg text-xs font-black uppercase tracking-widest shadow-sm transition-all focus:ring-2 focus:ring-offset-1 outline-none flex items-center justify-center gap-1.5 ${mainActionStyle}`}
                                        >
                                            {mainActionText} <ArrowRight size={14} />
                                        </button>
                                    </div>
                                </div>
                            </div>
